...
local bagshuiEnvironment = {
    ...
    periodicTable = function(item, displayType)
      ...
      if displayType ~= Bagshui.environment.BS_ITEM_INFO_DISPLAY_TYPE.TEXT then
        return string.format(
          Bagshui.environment.L.Suffix_Sets,
          table.getn(Bagshui.components.ItemInfo.periodicTableSetCache)
        )
      end
  ...
  pfUI = (_G.IsAddOnLoaded("pfUI") and _G.pfUI and _G.pfUI.uf and _G.pfUI.env and _G.pfUI.env.C) and _G.pfUI or false,
...
function Bagshui:Init()
  ...
  self.menuFrame = _G.CreateFrame("Frame", "BagshuiMenuHostFrame", nil, "UIDropDownMenuTemplate")
...
function Bagshui:AddonLoaded()
  ...
  then
    ...
    if
      _G.BagshuiData ~= nil
      and _G.BagshuiData[dataVersionKey] ~= nil
      and _G.BagshuiData[dataVersionKey] ~= BS_DATA_VERSION
    then
      self:Print(string.format(L.BagshuiDataReset, tostring(_G.BagshuiData[dataVersionKey]), BS_DATA_VERSION))
    end
