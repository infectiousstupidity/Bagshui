...
Bagshui:AddComponent(function()
  ...
  function Bagshui:FormatTooltipLine(text, label, title, indent)
    local tooltipText = (title and HIGHLIGHT_FONT_COLOR_CODE or NORMAL_FONT_COLOR_CODE)
      .. (text or "")
  ...
  function Bagshui:ShowTooltipAfterDelay(
    tooltip,
    expectedOwner,
    tooltipGroupElement,
    delayOverride,
    noTooltipDelayShorting,
    postDisplayCallback
  )
    ...
    if
      ...
      and _G.GetTime() - tooltipGroupElement.bagshuiData._showTooltipAfterDelay_LastShown
        < BS_TOOLTIP_DELAY_SECONDS.USE_SHORTENED_AFTER_LAST_SHOW_FOR
    then
  ...
  function Bagshui:SetInfoTooltipPosition(owner, managedElsewhere, forceBelow, attachToTooltip, infoTooltip)
    ...
    if
      not forceBelow
      and (
        (attachToTooltip:GetTop() and _G.GetScreenHeight() - attachToTooltip:GetTop() > attachToTooltip:GetBottom())
        or attachToTooltip.bagshuiData.displayNextAbove
      )
    then
      -- Above.
  ...
  function Bagshui:ShowInfoTooltip(infoTooltip)
    ...
    tooltip.bagshuiData.originalPoint, tooltip.bagshuiData.originalAnchorToFrame, tooltip.bagshuiData.originalAnchorToPoint, tooltip.bagshuiData.originalXOffset, tooltip.bagshuiData.originalYOffset =
      tooltip:GetPoint(1)
    ...
    if topTooltip:GetTop() / _G.UIParent:GetEffectiveScale() >= _G.UIParent:GetTop() then
      ...
      bottomTooltip:SetPoint("TOP" .. leftRight, topTooltip, "BOTTOM" .. leftRight, 0, BsSkin.infoTooltipYOffset)
    elseif bottomTooltip:GetBottom() / _G.UIParent:GetEffectiveScale() <= _G.UIParent:GetBottom() then
      ...
      topTooltip:ClearAllPoints()
      bottomTooltip:ClearAllPoints()
      topTooltip:SetPoint("BOTTOM" .. leftRight, bottomTooltip, "TOP" .. leftRight, 0, -BsSkin.infoTooltipYOffset)
    end
  end
  ...
  function Bagshui:ManageInfoTooltip(tooltip)
    ...
    then
      ...
      if
        tooltip.bagshuiData
        and tooltip.bagshuiData.infoTooltip
        and not tooltip.bagshuiData.infoTooltipManagedElsewhere
      then
    ...
    then
      ...
      if tooltip.bagshuiData and not tooltip.bagshuiData.infoTooltipManagedElsewhere then
        ...
        if
          ...
          and (
            (tooltip.bagshuiData.lastOwner and tooltip.bagshuiData.infoTooltip:IsOwned(tooltip.bagshuiData.lastOwner))
    ...
    then
      ...
      if
        ...
        and (_G.IsAltKeyDown() or tooltip.bagshuiData.showInfoTooltipWithoutAlt or BsSettings.showInfoTooltipsWithoutAlt)
      ...
      elseif
        not tooltip.bagshuiData.lastOwner or tooltip.bagshuiData.infoTooltip:IsOwned(tooltip.bagshuiData.lastOwner)
      then
        -- Need to hide.
        tooltip.bagshuiData.infoTooltip:Hide()
      end
  ...
  local hookTooltipFunctions = {
    ...
    SetOwner = function(self, owner, anchor, xOffset, yOffset)
      ...
      if _G.IsAddOnLoaded("aux-addon") then
        ...
        self.bagshuiData.lastItemString =
          BsItemInfo:ParseItemLink((parent and parent.row and parent.row.record and parent.row.record.link))
