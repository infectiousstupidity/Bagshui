...
Bagshui:AddComponent(function()
  ...
  function Bank:BagSlotButton_Init(bagSlotButton)
    ...
    bagSlotButton:SetScript("OnClick", function(bagButton, mouseButton)
      ...
      elseif not self.containers[this.bagshuiData.bagNum].purchased then
        return
      elseif not _G.IsAltKeyDown() and not _G.CursorHasItem() then
  ...
  function Bank:UpdateBagSlotPurchaseStatus(bagSlotButton, numSlotsPurchased, allSlotsPurchased)
    ...
    self.containers[bagSlotButton.bagshuiData.bagNum].purchased = (
      allSlotsPurchased or bagSlotButton.bagshuiData.bagSlotNum <= numSlotsPurchased
    )
    self.containers[bagSlotButton.bagshuiData.bagNum].nextPurchasable = (
      not allSlotsPurchased and bagSlotButton.bagshuiData.bagSlotNum == numSlotsPurchased + 1
    )
    -- `usable` is consumed by the "Equip Bag" item menu entry to determine when
    -- a slot should be disabled.
    self.containers[bagSlotButton.bagshuiData.bagNum].usable =
      self.containers[bagSlotButton.bagshuiData.bagNum].purchased
  end
    ...
    for _, bagSlotButton in ipairs(self.ui.buttons.bagSlots) do
      ...
      if not bagSlotButton.bagshuiData.primary then
        ...
        if bagSlotButton.bagshuiData.buttonComponents.background then
          bagSlotButton.bagshuiData.buttonComponents.background:SetVertexColor(
            textureColor[1],
            textureColor[2],
            textureColor[3]
          )
