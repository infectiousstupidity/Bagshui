  ...
  function Categories:InitUi()
    ...
    for _, inventoryType in ipairs(BS_INVENTORY_TYPE_UI_ORDER) do
      ...
      for _, containerId in ipairs(inventory.containerIds) do
        ...
        table.insert(BsRules.ruleFunctionTemplates.Bag, {
          -- Example: "Bank: Bag(10)"
          text = string.format(
            "%s%s:%s %s",
            NORMAL_FONT_COLOR_CODE,
            inventory.inventoryTypeLocalized,
            FONT_COLOR_CODE_CLOSE,
            ruleFunctionCode
          ),
          code = ruleFunctionCode,
          description = string.format(
            L.RuleFunction_Bag_ExampleDescription,
            containerId,
            sequentialBagSlotNum,
            inventory.inventoryTypeLocalized
          ),
        })
    ...
    local function insertRuleFunctionTemplate(arg1, arg2)
      ...
      if string.len(editBoxContent) > 0 then
        if not (string.find(editBoxContent, "%s+or%s*$") or string.find(editBoxContent, "%s+and%s*$")) then
    ...
    local function buildRuleFunctionMenu(
      ruleFunctionMenus,
      sortedRuleFunctionTemplateNames,
      ruleFunctionTemplates,
      ruleFunctionTemplateExtraEntries,
        ...
        if templates then
          ...
          if table.getn(templates) == 1 then
            ...
            repeat
              ...
              -- Auto-split submenu.
              Bagshui.prototypes.Menus:AddAutoSplitMenu(autoSplitMenuType, {
              ...
              table.insert(ruleFunctionMenu, {
                ...
                tooltipText = (ruleFunctionTemplateGenericDescriptions[functionName] or "")
                  .. (templates.aliasTooltipAddendum or ""),
                hasArrow = true,
                notCheckable = true,
                value = {
    ...
    function self.objectManager:NewEditor()
      ...
      repeat
        ...
        if categories.menus.menuList[menuName] and table.getn(categories.menus.menuList[menuName].levels[1]) > 0 then
      ...
      editor.ui:SetPoint(
        editor.idDisplay.bagshuiData.widget,
        "LEFT",
        editor.idDisplay.bagshuiData.labelFrame,
        "RIGHT",
        0,
        0
      )
    ...
    function categoryEditor:UpdateState(forceRuleValidation)
      ...
      elseif ruleEditBox.bagshuiData.hasFocus and not self.dirtyFields.rule then
      ...
      if ruleValidationButtonEnabled and ruleValid ~= nil then
        if ruleValid then
          ...
          Bagshui:Log(
            Categories:FormatErrorMessage(errorMessage, self.objectId, self.updatedObject.name),
            L.CategoryEditor,
      ...
      self.ui:SetIconButtonTexture(
        ruleValidationButton,
        "Icons\\" .. ruleValidationButtonTexture,
        ruleValidationButtonColor
      )
    ...
    local function classCategoryObjectIsEmpty(classObject)
      ...
      return (classObject == nil) or (string.len(classObject.rule or "") == 0 and table.getn(classObject.list) == 0)
    end
        ...
        for class, _ in pairs(BsGameInfo.characterClasses) do
          ...
          if originalClassObject or updatedClassObject then
            ...
            if self:IsObjectDirty(originalClassObject, updatedClassObject) then
              ...
              if
                not (classCategoryObjectIsEmpty(originalClassObject) and classCategoryObjectIsEmpty(updatedClassObject))
              then
