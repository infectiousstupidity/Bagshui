...
Bagshui:AddComponent(function()
  ...
  function Inventory:UpdateCache()
    ...
    local preCharges, preCount, preItemLink, preLocked, preItemString, preTooltip, nowCount, nowItemLink, nowLocked, nowCharges
...
bagInfo.numSlots > 0 and (not bagInfo.name or not bagInfo.type)
        )
      ...
      then
        ...
        bagInfo.isProfessionBag = (
          bagType ~= BsGameInfo.itemSubclasses["Container"]["Bag"] and bagType ~= self.primaryContainer.name
        )
      ...
      if bagNumSlots > 0 then
        ...
        for slotNum = 1, bagNumSlots do
          ...
          then
            ...
            then
              if item.itemString ~= preItemString and item.bagshuiDate == -1 then
                ...
                majorChanges = true
              elseif item.itemString == preItemString and item.count ~= preCount and preCount ~= -1 then
          ...
          if item.emptySlot ~= 1 then
            ...
            elseif
              Bagshui.pickedUpItemBagNum
              and Bagshui.pickedUpItemSlotNum
              and self.myContainerIds[Bagshui.pickedUpItemBagNum]
              and Bagshui.putDownItemBagNum
              and Bagshui.putDownItemSlotNum
              ...
              and (
                ...
                or (item.bagNum == Bagshui.putDownItemBagNum and item.slotNum == Bagshui.putDownItemSlotNum)
              )
            then
              -- Item moved.
...
item._proposedStockState ~= BS_ITEM_STOCK_STATE.DOWN
    ...
    -- (See Bagshui:PickupInventoryItem() for reasoning.)
    if hasPendingContainerChanges and Bagshui.pickedUpBagSlotNum and Bagshui.putDownBagSlotNum then
