...
Bagshui:AddComponent(function()
  ...
  function Inventory:ShowEditModeCursor()
    if self.editMode and self.editState.cursorItem ~= nil and self.editState.cursorTooltipTitle ~= nil then
  ...
  function Inventory:FormatEditModeCursorTooltipTitle(objectName, objectType)
    return BS_FONT_COLOR.BAGSHUI
      .. string.format(L.Symbol_Colon, objectType)
  ...
  function Inventory:EditModePickUpItem(item)
    ...
    self.editState.cursorTooltipTitle =
      self:FormatEditModeCursorTooltipTitle(BsItemInfo:GetQualityColoredName(item), L.Item)
  ...
  function Inventory:ShowItemCategoryAssignmentMenuForGroup(item, groupWidget)
    ...
    self.menus:OpenMenu("Item", item, groupId, groupWidget.mouseIsOver and "cursor" or groupWidget)
  end
  ...
  function Inventory:EditModePickUpGroup(groupId)
    ...
    self.editState.cursorTooltipTitle = self:FormatEditModeCursorTooltipTitle(
      ((string.len(self.groups[groupId].name or "") > 0) and self.groups[groupId].name or L.UnnamedGroup),
      L.Group
    )
    self.editState.cursorTooltipText = string.format(L.EditMode_Tooltip_SelectNew, string.lower(L.Location))
  ...
  function Inventory:AssignGroupToLayout(objectId, objectType, insertType, destRow, destColumn)
    ...
    if objectType == BS_INVENTORY_OBJECT_TYPE.GROUP then
      ...
      if
        ...
        and (destColumn == sourceColumn or destColumn == sourceColumn + 1)
      then
        return
      end
      ...
      if objectType == BS_INVENTORY_OBJECT_TYPE.CATEGORY or objectType == BS_INVENTORY_OBJECT_TYPE.ITEM then
  ...
  function Inventory:ManagePendingGroupAssignments(groupId, apply)
    if apply then
      ...
      elseif objectType == BS_INVENTORY_OBJECT_TYPE.ITEM then
        self:ShowItemCategoryAssignmentMenuForGroup(objectId, self.groupsIdsToFrames[groupId])
      end
    end
  ...
  function Inventory:RenameGroup(groupId, groupIsNew)
    ...
    local prompt = groupIsNew and L.EditMode_Prompt_NewGroupName
      or string.format(
        L.EditMode_Prompt_RenameGroup,
        (string.len(currentName or "") > 0 and currentName or L.UnnamedGroup)
      )
