...
Bagshui:AddComponent(function()
  ...
  function Inventory:ManageDryRun(phase1)
    if phase1 then
      ...
      if (not self.resortNeeded or self:Visible()) and not self.forceResort then
        self.dryRun = true
  ...
  function Inventory:UpdateWindow()
    ...
    self.uiFrame:SetBackdropColor(self.windowColor[1], self.windowColor[2], self.windowColor[3], self.windowColor[4])
    ...
    if self.windowUpdateNeeded then
      ...
      local groupPadding = self.settings.groupPadding
        + (BsSkin.groupPaddingFudge or 0)
        + ((BsSkin.itemSlotMarginFudge / 2) * itemSlotScale)

      local maxColumns = self.settings.windowMaxColumns
      local anchorLeft = self.settings.windowContentAlignment == "LEFT"
      local showGroupLabels = (self.settings.showGroupLabels and not self.settings.hideGroupLabelsOverride)
        or self.editMode
      ...
      if self.editMode then
        ...
        rowGroupMoveTargetWidth = adjustedItemSlotSize
          + (itemSlotMargin * 2)
          + (groupPadding * 2)
...
(self.groupWidthsInItems[group.groupId] * (adjustedItemSlotSize + itemSlotMargin))
          ...
          for columnNum = rowGroupStart, rowGroupEnd, rowGroupStep do
            ...
            if self.groupWidthsInItems[groupId] > 0 then
              ...
              if showGroupLabels then
                ...
                if self.editMode then
                  group.bagshuiData.labelFrame:SetWidth(
                    self.actualGroupWidths[groupId] - (groupPadding * 2) - itemSlotMargin
                  )
                  group.bagshuiData.labelFrame:SetPoint(
                    "TOPLEFT",
                    group,
                    "TOPLEFT",
                    BsSkin.groupLabelEditModeXOffset,
                    BsSkin.groupLabelEditModeYOffset
                  )
                  group.bagshuiData.labelFrame:EnableMouse(false)
                else
                  group.bagshuiData.labelFrame:SetWidth(
                    self.actualGroupWidths[groupId] + BsSkin.groupLabelRelativeWidth
                  )
                  group.bagshuiData.labelFrame:SetPoint(
                    "BOTTOMLEFT",
                    group,
                    "TOPLEFT",
                    BsSkin.groupLabelXOffset,
                    BsSkin.groupLabelYOffset
                  )
          ...
          currentRowLastFrameOutsideX = (
            anchorLeft and self.positioningTables.UpdateWindow.anchorToFrame:GetRight()
            or self.positioningTables.UpdateWindow.anchorToFrame:GetLeft()
          )
          if
            widestRowLastFrameOutsideX == 0
            or (
              anchorLeft and widestRowLastFrameOutsideX < currentRowLastFrameOutsideX
      ...
      finalWindowHeight =
        -- Inventory content.
        ((topmostFrame and firstFrame) and (topmostFrame:GetTop() - firstFrame:GetBottom()) or 0)
        -- Bottom group margin.
        + groupEdgeYOffset
        -- Edit Mode messes with the sizing.
        + (
          self.editMode
            and BsSkin.groupMoveTargetThinDimension
          or (
...
groupEdgeOffset
      ...
      if showHeader then
        ...
        finalWindowHeight = finalWindowHeight
      ...
      if showFooter then
        ...
        local bagBarScale = (itemSlotSize / uiButtons.itemSlots[1].bagshuiData.originalSizeAdjusted)
          * BsSkin.bagBarScale
...
self.settings.showBagBar and (uiButtons.itemSlots[1].bagshuiData.originalSizeAdjusted * bagBarScale)
  ...
  function Inventory:GetGroupItemCountForLayout(groupId, zeroUnlessHasItemsInBagNum)
    ...
    for _, item in ipairs(self.groupItems[groupId]) do
      ...
      elseif self.hideItems[item.id] then
        ...
        itemVisible = (self.editMode or self.showHidden or self.highlightItemsInContainerLocked == item.bagNum)
      end
  ...
  function Inventory:ShowFrameInNextPosition(positioningTableName, rowNum, frame, width, height, scale)
    local positioningTable = self.positioningTables[positioningTableName]

    assert(
      positioningTable,
      "Inventory:ShowFrameInNextPosition(): Positioning table '"
        .. tostring(positioningTableName)
        .. "' has not been initialized; must call InitFramePositioningTable() first!"
    )
  ...
  function Inventory:SizeAndShowFrame(
    ...
    assert(
      sizeAndShow_frame ~= nil,
      "Invalid frame '" .. tostring(frameObjOrName) .. "' provided to SizeAndShowFrame()."
    )
    ...
    if
      not scale
      and sizeAndShow_frame.bagshuiData
      and sizeAndShow_frame.bagshuiData.type == BS_UI_ITEM_BUTTON_TYPE.ITEM
    then
  ...
  function Inventory:ShowNextGroupMoveTarget(width, height, uiRowNum, layoutRowNum, layoutColumnNum, insertType)
    ...
    button:SetPoint("TOPLEFT", groupMoveTarget, "TOPLEFT", -xOffsetLeft, -yOffset)
    button:SetPoint("BOTTOMRIGHT", groupMoveTarget, "BOTTOMRIGHT", xOffsetRight, yOffset)

    -- Display the frame.
    self:ShowFrameInNextPosition("UpdateWindow", uiRowNum, groupMoveTarget, width, height)
  end
  ...
  function Inventory:UpdateBagBar()
    ...
    for _, bagSlotButton in ipairs(self.ui.buttons.bagSlots) do
      ...
      elseif self.highlightItemsInContainerLocked ~= bagSlotButton.bagshuiData.bagNum then
        ...
        bagSlotButton.bagshuiData.buttonComponents.innerGlow:SetVertexColor(0, 0, 0, 0)
      ...
      bagSlotButton.bagshuiData.buttonComponents.iconTexture:SetVertexColor(
        ...
        (not self.online and BsSkin.itemSlotTextureOfflineOpacity or 1)
      )

      -- Overall button opacity.
      bagSlotButton:SetAlpha(
        self.editMode and 0.2
...
self.hoveredItem._bagsRepresented
          ...
          or ((self.highlightItemsInContainerLocked and self.highlightItemsInContainerLocked ~= bagSlotButton.bagshuiData.bagNum) and 0.7)
    ...
    if self.alwaysShowUsageSummary or self.ui.frames.spaceSummary.bagshuiData.mouseIsOver then
      showUsageSummary = true
    end
    ...
    if textParts[2] then
      ...
      elseif textParts[3] == "Total" then
        subText =
          string.format("%s/%s", (textParts[2] == "Used" and self.usedSlots or self.availableSlots), self.totalSlots)
    ...
    if showUsageSummary then
      ...
      self.ui.frames.spaceSummary.bagshuiData.text:SetTextColor(
        self.settings.groupLabelDefault[1],
        self.settings.groupLabelDefault[2],
        self.settings.groupLabelDefault[3],
        self.settings.groupLabelDefault[4]
      )
      self.ui.frames.miniSpaceSummaryBottom.bagshuiData.text:SetText(miniText)
      self.ui.frames.miniSpaceSummaryBottom.bagshuiData.text:SetTextColor(
        self.settings.groupLabelDefault[1],
        self.settings.groupLabelDefault[2],
        self.settings.groupLabelDefault[3],
        self.settings.groupLabelDefault[4]
      ...
      self.ui.frames.miniSpaceSummaryTop.bagshuiData.text:SetTextColor(
        self.settings.groupLabelDefault[1],
        self.settings.groupLabelDefault[2],
        self.settings.groupLabelDefault[3],
        self.settings.groupLabelDefault[4]
      ...
      if subText then
        ...
        self.ui.frames.spaceSummary.bagshuiData.subtext:SetTextColor(
          self.settings.groupLabelDefault[1],
          self.settings.groupLabelDefault[2],
          self.settings.groupLabelDefault[3],
          self.settings.groupLabelDefault[4]
        ...
        self.ui.frames.miniSpaceSummaryBottom.bagshuiData.subtext:SetTextColor(
          self.settings.groupLabelDefault[1],
          self.settings.groupLabelDefault[2],
          self.settings.groupLabelDefault[3],
          self.settings.groupLabelDefault[4]
        ...
        self.ui.frames.miniSpaceSummaryTop.bagshuiData.subtext:SetTextColor(
          self.settings.groupLabelDefault[1],
          self.settings.groupLabelDefault[2],
          self.settings.groupLabelDefault[3],
          self.settings.groupLabelDefault[4]
      ...
      summaryWidth = math.max(
        self.ui.frames.spaceSummary.bagshuiData.text:GetStringWidth(),
        self.ui.frames.spaceSummary.bagshuiData.subtext:GetStringWidth()
      )
...
self.dockedInventory and self.dockedInventory.multiplePartialStacks
    ...
    for inventoryType, _ in self:OtherInventoryTypesInToolbarIconOrder(true) do
      self:SetToolbarButtonState(toolbarButtons[inventoryType], nil, true, Bagshui.components[inventoryType]:Visible())
    end
    -- Catalog.
    self:SetToolbarButtonState(toolbarButtons.catalog, nil, true, BsCatalog:Visible())
    ...
    if self.editMode then
      ...
      self.ui.text.status:SetText(
        NORMAL_FONT_COLOR_CODE
          .. string.format(L.Symbol_Colon, L.Profile_Structure)
      ...
      toolbarButtons.offline.bagshuiData.tooltipText = GRAY_FONT_COLOR_CODE
        ...
        for anchorPosition = widgetPosition - 1, 1, -1 do
          if type(widgetList[anchorPosition]) ~= "table" then
            ...
            if anchorButton:IsShown() then
              widget:SetPoint(anchorPoint, anchorButton, BsUtil.FlipAnchorPoint(anchorPoint), nextOffset, 0)
              break
            end
          end
