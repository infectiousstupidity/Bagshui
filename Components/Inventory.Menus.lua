  ...
  function Inventory:InitMenus()
    ...
    self._autoSplitMenuItem_Categories_TooltipTextFunc = function(categoryId)
      ...
      if self.categoriesToGroups[categoryId] and self.groups[self.categoriesToGroups[categoryId]] then
        ...
        tooltipText = string.format(
          L.EditMode_CategoryInGroup,
          self.groups[self.categoriesToGroups[categoryId]].name or L.Unnamed
        ) .. rowColumnString
      end
      return tooltipText
    end
    ...
    if toolbarInsertPoint then
      for i = table.getn(self.toolbarAndMainMenuItems), 1, -1 do
        ...
        table.insert(self.menus.menuList.Main.levels[1], toolbarInsertPoint, menuItem)
      end
    else
    ...
    for inventoryType, inventoryTypeLocalized in self:OtherInventoryTypesInToolbarIconOrder(false, true) do
      ...
      table.insert(self.menus.menuList.Main.levels[1], {
        ...
        _bagshuiHideFunc = function()
          ...
          return (inventoryClass == self and self.settings.showHeader)
        end,
        _bagshuiCheckedFunc = function()
        ...
        },
        {
          ...
          value = {
            ...
            disableFunc = function(categoryId, menuItem, triggeringMenuValue)
              if
                menuItem.checked
                and BsUtil.TableContainsValue(GROUP_CATEGORY_REMOVE_MENU_IDS_TO_DISABLE, categoryId) ~= nil
              then
                return true
              end
              return false
          ...
          value = {
            ...
            func = function(_, categoryId)
              ...
              self.menus:OpenMenu(
                ...
                nil,
                _G.UIParent,
                _G.DropDownList1:GetLeft(),
                _G.DropDownList1:GetTop()
              )
            end,
            tooltipTextFunc = self._autoSplitMenuItem_Categories_TooltipTextFunc,
          },
        ...
        },
        {
          ...
          value = {
            ...
            nameRevisionFunc = function(itemId, itemName)
              return (itemId == self.settings.defaultSortOrder) and string.format(L.Suffix_Default, itemName)
                or itemName
            end,
                  ...
                  tooltipTextFunc = function()
                    return string.format(
                      ...
                      LIGHTYELLOW_FONT_COLOR_CODE
                        .. BsSortOrders.list[self.settings.defaultSortOrder].name
      ...
      function(menu, uiGroup)
        ...
        for _, menuItem in ipairs(groupMenu.levels[1]) do
          if menuItem._bagshuiMenuItemId == "GroupNameTitle" then
            ...
            menuItem.text = string.len(group.name or "") > 0
                and (string.format(L.Symbol_Colon, L.Group) .. " " .. LIGHTYELLOW_FONT_COLOR_CODE .. BsUtil.TruncateString(
                  group.name,
                  45
                ) .. FONT_COLOR_CODE_CLOSE)
              or L.Group
          elseif type(menuItem.value) == "table" and menuItem.value.autoSplitMenuType then
            ...
            if menuItem.value.autoSplitMenuType == BS_AUTO_SPLIT_MENU_TYPE.CATEGORIES then
              ...
              elseif
                menuItem._bagshuiMenuItemId == "CategoryMove" or menuItem._bagshuiMenuItemId == "CategoryConfig"
              then
    ...
    self.menus:AddMenu(
      ...
      -- 1
      {
        ...
        -- in the OpenMenu callback function.
        {
          ...
          value = {
            ...
            tooltipTextFunc = function(categoryId)
              local layoutLocation = LIGHTYELLOW_FONT_COLOR_CODE
                .. self._autoSplitMenuItem_Categories_TooltipTextFunc(categoryId)
                .. FONT_COLOR_CODE_CLOSE
              return NORMAL_FONT_COLOR_CODE
                .. L.Menu_Item_MatchedCategory_TooltipText
                .. FONT_COLOR_CODE_CLOSE
                .. BS_NEWLINE
                .. layoutLocation
            end,
        ...
        -- OpenMenu callback function.
        {
          ...
          tooltipText = L.Menu_Item_AssignToCategory_TooltipText
            .. BS_NEWLINE
            .. LIGHTYELLOW_FONT_COLOR_CODE
            .. L.Menu_Item_AssignToCategory_Hint_CustomOnly
            .. FONT_COLOR_CODE_CLOSE,
          value = {
            autoSplitMenuType = BS_AUTO_SPLIT_MENU_TYPE.CATEGORIES,
            ...
            autoSplitMenuExtraItems = {
              {
                text = string.format(L.Symbol_Ellipsis, L.New),
                tooltipText = L.Menu_Item_AssignToCategory_CreateNew_TooltipText
                  .. BS_NEWLINE
                  .. LIGHTYELLOW_FONT_COLOR_CODE
                  .. L.Menu_Item_AssignToCategory_Hint_CustomOnly
                  .. FONT_COLOR_CODE_CLOSE,
      ...
      function(menu, item, groupId)
        ...
        local directAssignmentCategoryIds = (groupId and self.groups[groupId] and self.groups[groupId].categories)
          or nil
        ...
        BsCategories:GetDirectCategoryAssignmentsForItem(
          item.id,
          self._itemMenuDirectCategoryAssignmentList,
          directAssignmentCategoryIds
        )
        ...
        for _, menuItem in pairs(itemMenu.levels[1]) do
          ...
          if menuItem._bagshuiMenuItemId == "CategoryTitle" then
            ...
            menuItem.text = string.format(L.Symbol_Colon, L.Category)
              .. " "
              .. LIGHTYELLOW_FONT_COLOR_CODE
          ...
          elseif menuItem._bagshuiMenuItemId == "ItemNameTitle" then
            ...
            menuItem.text = string.format(L.Symbol_Colon, L.Item)
              .. " "
              .. LIGHTYELLOW_FONT_COLOR_CODE
              .. BsUtil.TruncateString(item.name, 45)
              .. FONT_COLOR_CODE_CLOSE
        ...
        if not directAssignmentOnlyMode then
          ...
          for _, itemPropertyFriendly, _, itemPropertyDisplay in BsItemInfo:ItemPropertyValuesForDisplay(item) do
            ...
            infoMenu[infoMenuItemCount].text = string.format(L.Symbol_Colon, itemPropertyFriendly)
              .. " "
              .. itemPropertyDisplay
            infoMenu[infoMenuItemCount].tooltipText = LIGHTYELLOW_FONT_COLOR_CODE
              .. L.Menu_Item_Information_Submenu_TooltipText
              .. FONT_COLOR_CODE_CLOSE
    ...
    self.menus:InitializeEmptyMenu(self.menus.menuList.Item.levels[2].itemInfo, {
      text = "",
      arg1 = "",
      func = showItemInfo,
    })
    ...
    self.menus:AddMenu(
      ...
      function(menu, itemAndGroupId, categoryId)
        ...
        for i = 4, table.getn(menu.levels[1]) do
          ...
          menuItem.checked =
            BsCategories:ItemInDirectAssignmentList(nil, categoryId, itemAndGroupId.item.id, nil, nil, menuItem.value)
        end
      end
    )
  end
