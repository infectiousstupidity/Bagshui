...
Bagshui:AddComponent(function()
  ...
  function InventoryUi:CreateBagSlotButtons()
    ...
    for index, containerId in ipairs(inventory.containerIds) do
      ...
      if table.getn(ui.buttons.bagSlots) == 0 then
        bagSlotButton:SetPoint("BOTTOMLEFT", 3, 0)
      else
        bagSlotButton:SetPoint("TOPLEFT", previousBagSlot, "TOPRIGHT", BsSkin.bagBarSpacing, 0)
      end
      ...
      bagSlotButton:SetScript("OnClick", function(bagButton, mouseButton)
        ...
        if
          ...
          and (
            ...
            or (inventory.highlightItemsInContainerLocked == bagNum and inventory.online)
          )
        ...
        then
          ...
          inventory.highlightItemsInContainerLocked = inventory.highlightItemsInContainerLocked ~= bagNum and bagNum
            or nil
  ...
  function Inventory:BagSlotButton_Init(bagSlotButton) end
  ...
  function Inventory:ShowBagSlotTooltip(bagSlotButton)
    ...
    if not this.bagshuiData.primary and self.online and this.bagshuiData.inventorySlotId then
    ...
    if
      ...
      or (not self.online and (self.containers[this.bagshuiData.bagNum].name or this.tooltipText or (self.activeCharacterId ~= Bagshui.currentCharacterId)))
    ...
    if this.hasItem and self.opensViaHooks and self.activeCharacterId == Bagshui.currentCharacterId then
      local keyBinding = _G.GetBindingKey(
        (
          this.bagshuiData.primary and self.primaryContainer.bindingKey
          or (
            self.keyBindingPrefix
            and table.getn(self.containerIdRange) > 0
            -- Refer to Blizzard's BagSlotButton_OnEnter.
            and self.keyBindingPrefix
              .. (self.containerIdRange[2] - (this.bagshuiData.containerId - self.containerIdRange[1]))
          )
        )
      )
      if keyBinding then
    ...
    then
      ...
      _G.GameTooltip:AddLine(
        string.format(L.Tooltip_Inventory_PlaceBagInBagHint, L.ShiftClick),
        GRAY_FONT_COLOR.r,
        GRAY_FONT_COLOR.g,
        GRAY_FONT_COLOR.b
      )
    ...
    if
      not self.editMode
      and (((self.activeCharacterId == Bagshui.currentCharacterId) and this.hasItem or false) or (self.containers[this.bagshuiData.bagNum].numSlots or 0) > 0)
    ...
    then
      _G.GameTooltip:AddLine(
        string.format(
          ...
          L.AltClick,
          string.lower(
            (self.highlightItemsInContainerLocked and self.highlightItemsInContainerId == this.bagshuiData.bagNum)
                and L.Unlock
              or L.Lock
          )
        ),
  ...
  function Inventory:GetBagSlotSwappableText(bagNum, noColors)
    ...
    if neededSlots > 0 then
      ...
      if
        ...
        type(self.bagSwappingSupplementalStorage) == "table" and table.getn(self.bagSwappingSupplementalStorage) > 0
      then
        ...
        for _, inventoryClassName in ipairs(self.bagSwappingSupplementalStorage) do
          if not inventoriesChecked[inventoryClassName] then
            supplementalClasses = (string.len(supplementalClasses) > 0 and "/" or "")
              .. supplementalClasses
              .. L[inventoryClassName]
          end
        end
