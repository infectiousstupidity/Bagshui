...
Bagshui:AddComponent(function()
  ...
  function Inventory:Group_OnEnter(group)
    ...
    if inventory.editMode and not inventory:EditModeCursorHasItem() then
      ...
      local groupName = string.len(inventory.groups[groupId].name or "") > 0 and inventory.groups[groupId].name
        or L.UnnamedGroup
        ...
        for _, categoryId in ipairs(categoryList) do
          if BsCategories.list[categoryId] then
            categories = categories
              ...
              .. (BsCategories:GetName(categoryId) or tostring(categoryId))
              .. FONT_COLOR_CODE_CLOSE
              .. BS_NEWLINE
          end
        end
      ...
      inventory:AddBagshuiInfoTooltipLine(
        L.RightClick,
        string.format(L.Symbol_Colon, string.format(L.Suffix_Menu, L.Group))
      )
    ...
    if mouseButton == "LeftButton" or inventory:EditModeCursorHasItem() then
      ...
      if inventory:EditModeCursorHasItem() then
        ...
        elseif inventory.editState.cursorItemType == BS_INVENTORY_OBJECT_TYPE.CATEGORY then
          inventory:AssignCategoryToGroup(inventory.editState.cursorItem, this.bagshuiData.groupId)
        elseif inventory.editState.cursorItemType == BS_INVENTORY_OBJECT_TYPE.ITEM then
          inventory:ShowItemCategoryAssignmentMenuForGroup(inventory.editState.cursorItem, this)
        else
          -- Failsafe
  ...
  function InventoryUi:CreateGroup(groupNum)
    ...
    if not ui.CreateGroupIfNotExists then
      ...
      function ui.CreateGroupIfNotExists(elementNum)
        ...
        ui.frames.groups[elementNum] =
          _G.CreateFrame("Button", ui:CreateElementName("Group" .. elementNum), ui.frames.main)
    ...
    ui:CreateIfNotExists(groupNum, ui.frames.groups, ui.CreateGroupIfNotExists)
  ...
  function InventoryUi:SetGroupColors(uiGroup, mouseDown)
    ...
    if uiGroup.bagshuiData.type == BS_INVENTORY_OBJECT_TYPE.GROUP then
      ...
      textColor = inventory.groups[groupId] and inventory.groups[groupId].label or inventory.settings.groupLabelDefault
  ...
  function InventoryUi:CreateGroupMoveTarget(groupMoveTargetNum)
    ...
    if not ui.CreateGroupMoveTargetIfNotExists then
      function ui.CreateGroupMoveTargetIfNotExists(elementNum)
        ui.frames.groupMoveTargets[elementNum] =
          _G.CreateFrame("Frame", ui:CreateElementName("GroupMoveTarget" .. elementNum), ui.frames.main)
    ...
    ui:CreateIfNotExists(groupMoveTargetNum, ui.frames.groupMoveTargets, ui.CreateGroupMoveTargetIfNotExists)
  end
