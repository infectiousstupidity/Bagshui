...
Bagshui:AddComponent(function()
  ...
  function Inventory:ItemButton_OnEnter(itemButton)
    ...
    if
      self.editMode
      or _G.IsAltKeyDown()
      or (BsSettings.showInfoTooltipsWithoutAlt and not _G.IsShiftKeyDown())
      ...
      if not self.editMode then
        ...
        if editModeShowItemTooltip then
          ...
          if item.id > 0 then
            self:AddBagshuiInfoTooltipLine(
              L.AltClick,
              string.format(L.Symbol_Colon, string.format(L.Prefix_Move, L.Item))
            )
          end
          self:AddBagshuiInfoTooltipLine(
            L.RightClick,
            string.format(L.Symbol_Colon, string.format(L.Suffix_Menu, L.CategorySlashItem))
          )
          ...
          self:AddBagshuiInfoTooltipLine(
            L.ReleaseAlt,
            string.format(L.Symbol_Colon, string.format(L.Prefix_Target, L.Category))
          )
        else
          ...
          self:AddBagshuiInfoTooltipLine(
            L.Click,
            string.format(L.Symbol_Colon, string.format(L.Prefix_Move, L.Category))
          )
          ...
          if string.len(item.name or "") > 0 then
            self:AddBagshuiInfoTooltipLine(
              ...
              string.format(L.Symbol_Colon, L.Item),
              true
            )
          end
          ...
          self:AddBagshuiInfoTooltipLine(
            L.HoldAlt,
            string.format(L.Symbol_Colon, string.format(L.Prefix_Target, L.Item))
          )
        end
      end
    ...
    if
      (
        ...
        or (BsInfoTooltip:IsVisible() and _G.MouseIsOver(BsInfoTooltip, offset, offset, offset, -offset))
  ...
  function Inventory:ItemButton_OnClick(mouseButton, isDrag, itemButton)
    ...
    if self.editMode then
      ...
      if item then
        ...
        if self.itemPendingSale and (mouseButton == "LeftButton" or self.itemPendingSale ~= item) then
...
(self.itemPendingSale == item and mouseButton == "RightButton" and _G.IsControlKeyDown())
...
_G.IsAddOnLoaded("Postal")
  ...
  function Inventory:IsItemClickActionAllowed(mouseButton, frame1, frame2, frame3, frame4)
    return not _G.CursorHasItem()
      and ((self.settings.altClickAttach and mouseButton == "LeftButton" and _G.IsAltKeyDown()) or (self.settings.rightClickAttach and mouseButton == "RightButton"))
      and (
