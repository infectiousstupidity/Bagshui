...
Bagshui:AddComponent(function()
  ...
  function Inventory:InitUi()
    ...
    frames.searchBox.bagshuiData.searchIcon.bagshuiData.tooltipText =
      string.format(L.Toolbar_Search_TooltipText, self.inventoryTypeLocalized)
    ...
    frames.bagBar = _G.CreateFrame("Frame", ui:CreateElementName("BagBar"), footer)
    ...
    frames.bagBar.bagshuiData = {
      baseWidth = (buttons.itemSlots[1].bagshuiData.originalSizeAdjusted + BsSkin.bagBarSpacing)
        * table.getn(self.containerIds),
    }
    ...
    local function CreateSpaceSummary(parent, justifyH)
      ...
      spaceSummary:SetHitRectInsets(
        -(BsSkin.bagBarSpacing / 2),
        -(BsSkin.bagBarSpacing / 2),
        -(BsSkin.bagBarSpacing / 2),
        -(BsSkin.bagBarSpacing / 2)
      )
    ...
    frames.spaceSummary:SetPoint(
      "LEFT",
      buttons.bagSlots[table.getn(buttons.bagSlots)],
      "RIGHT",
      BsSkin.bagBarSpacing - 2,
      0
    )
  ...
  function Inventory:ShowUsageSummary(owner, attachedToBagBar, tooltip, extraText, delay)
    ...
    tooltip:AddDoubleLine(
      ...
      string.format(L.Symbol_Colon, L.Available)
        .. " "
        .. tostring(self.availableSlots),
      string.format(L.Symbol_Colon, L.Used)
        .. " "
        .. string.format("%s/%s", tostring(self.usedSlots), tostring(self.totalSlots))
    )
  ...
  function Inventory:SetDockedToFrameVisibility(visibilityAction)
    ...
    if
      ...
      or (visibilityAction == BS_INVENTORY_UI_VISIBILITY_ACTION.CLOSE and not self.dockingFrameVisibleOnLastOpen)
    then
  ...
  function Inventory:RescueWindow(requested)
    ...
    if rescued then
      Bagshui:Print(
        requested and L.SettingReset_WindowPositionManual or L.SettingReset_WindowPositionAuto,
        self.inventoryTypeLocalized
      )
  ...
  function Inventory:GetWindowOffset(point)
    ...
    local offset = self.uiFrame["Get" .. BsUtil.Capitalize(point)](self.uiFrame) * self.uiFrame:GetScale()
