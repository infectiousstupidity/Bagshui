...
Bagshui:AddComponent(function()
  ...
  function Inventory:New(newPropsOrInventoryType)
    ...
    BsSlash:AddOpenCloseHandler(classObj.inventoryType, classObj, {
      L.Settings,
      L.ResetPosition,
      L.Lock,
      L.Unlock,
      ...
      if BsUtil.MatchLocalizedOrNon(tokens[2], "lock") or BsUtil.MatchLocalizedOrNon(tokens[2], "unlock") then
  ...
  function Inventory:Init()
    ...
    self.currentCharacterInventory =
      Bagshui.characters[Bagshui.currentCharacterId][self.inventoryTypeSavedVars].inventory
  ...
  function Inventory:OnEvent(event, arg1, arg2)
    ...
    if event == "BAGSHUI_SETTING_UPDATE" then
      ...
      if
        arg2
        and (arg2.inventoryCacheUpdateOnChange or arg2.inventoryResortOnChange or arg2.inventoryWindowUpdateOnChange)
      then
        -- Reset header/footer show/hide double-click state when the corresponding setting is changed.
        if (arg1 == "showHeader" or arg1 == "showFooter") and not self.ignoreNextSettingChange then
    ...
    if event == "BAGSHUI_INVENTORY_EDIT_MODE_UPDATE" then
      -- arg1 = Raising class.
      -- arg2 = forceResort.
      if arg1 ~= self and arg1.settings.profileStructure == self.settings.profileStructure then
