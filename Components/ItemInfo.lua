...
Bagshui:LoadComponent(function()
  ...
  function ItemInfo:Get(itemIdentifier, itemInfoTable, reinitialize, initialize, forceIntoLocalGameCache, inventory)
    ...
    local itemName, itemTexture, itemString, itemStringFromGetItemInfo, itemQuality, itemMinLevel, itemType, itemSubtype, itemMaxStackCount, itemEquipLocation, itemId, itemLinkWithoutRandomSuffix, itemBaseName, itemSuffixName
    ...
    if item.itemString ~= nil then
      ...
      if item.equipLocation and string.len(item.equipLocation) > 0 then
        ...
        item.equipLocationSort = BS_INVENTORY_EQUIP_LOCATION_SORT_ORDER[item.equipLocation]
          or BS_ITEM_SKELETON.equipLocationSort
      end
      item.qualityLocalized = _G[string.format("ITEM_QUALITY%d_DESC", item.quality)]
        or BS_ITEM_SKELETON.qualityLocalized
  ...
  function ItemInfo:InitializeItem(itemInfoTable, initializeProtected, validate, temp)
    ...
    for itemKey, defaultValue in pairs(BS_ITEM_SKELETON) do
      if
        (itemInfoTable[itemKey] == nil and not BS_ITEM_NIL_PROPERTIES[itemKey])
        or (
                    -- Don't reset properties with values during validation.
not validate
          -- Never reset a protected property unless forced.
          and not (BS_ITEM_PROTECTED_PROPERTIES[itemKey] and not initializeProtected)
        )
      then
  ...
  function ItemInfo:InitializeEmptySlotItem(item, isEmptySlotStack)
    ...
    item.name = (isEmptySlotStack and item.bagType ~= BsGameInfo.itemSubclasses.Container.Bag)
  ...
  function ItemInfo:ParseItemLink(itemLink)
    ...
    local found, _, itemId, enchantId, suffixId, uniqueId =
      string.find(tostring(itemLink), "item:(%d+):(%d+):(%d+):(%d+)")
    if found then
  ...
  function ItemInfo:LoadTooltip(tooltipFrame, item, inventory, forceItemString)
    ...
    local itemString = type(item) == "string" and item
    ...
    elseif type(item) == "table" then
      ...
      if inventory and inventory.primaryContainer.id == item.bagNum and inventory.getInventorySlotFunction then
        -- SetInventoryItem() returns hasItem, hasCooldown, repairCost.
        _, hasCooldown, repairCost =
          tooltipFrame:SetInventoryItem("player", inventory.getInventorySlotFunction(item.slotNum))
      else
  ...
  function ItemInfo:AddTooltipInfo(item, tooltip)
    ...
    for _, itemPropertyFriendly, itemPropertyValue, itemPropertyDisplay in
      self:ItemPropertyValuesForDisplay(item, BS_ITEM_INFO_DISPLAY_TYPE.TOOLTIP)
    do
      ...
      for ttLineNum = 1, BsHiddenTooltip:NumLines() do
        ...
        for i, lr in ipairs(ttBagshuiData.textFieldsPerLine) do
          ...
          if ttTextFrame ~= nil and ttTextFrame:IsVisible() then
            ...
            if r == _G.RED_FONT_COLOR.r and g == _G.RED_FONT_COLOR.g and b == _G.RED_FONT_COLOR.b then
  ...
  function ItemInfo:IsContainer(item)
    return (type(item) == "table" and (item.type == L.Container or item.type == L.Quiver))
  end
  ...
  function ItemInfo:GetQualityColoredName(item)
    ...
    return _G.ITEM_QUALITY_COLORS[item.quality or 1].hex
  ...
  function ItemInfo:GetUniqueItemId(item, bagNumOverride, slotNumOverride)
    return (bagNumOverride or item.bagNum or "?")
      .. ":"
  ...
  function ItemInfo:BuildRuleFunctionExampleText(ruleFunctionExamples, itemPropertyValue, itemPropertyFriendly)
    ...
    for _, ruleFunctionExample in ipairs(ruleFunctionExamples) do
      ...
      local ruleFunctionParameter = itemPropertyValue ~= nil and string.gsub(tostring(itemPropertyValue), '"', '\\"')
        or ("<" .. tostring(itemPropertyFriendly) .. ">")
    ...
    return function()
      ...
      local itemPropertyValue = (
        (
          ...
          and (realtimeItemProperties[itemProperty] and realtimeItemProperties[itemProperty](item, displayType) or (item[itemProperty] ~= nil and item[itemProperty] ~= "") and tostring(
            item[itemProperty]
          ) or nil)
        or nil
      )
      ...
      local itemPropertyDisplay = itemPropertyValue
          and (type(itemPropertyValue) ~= "table" and (NORMAL_FONT_COLOR_CODE .. (displayType ~= Bagshui.environment.BS_ITEM_INFO_DISPLAY_TYPE.TEXT and BsUtil.TruncateString(
            tostring(itemPropertyValue),
            35
          ) or tostring(itemPropertyValue)) .. FONT_COLOR_CODE_CLOSE) or "")
  ...
  function ItemInfo:Init()
    ...
    function self.window:Open(item)
      ...
      if type(item) == "table" then
        ...
        for itemProperty, itemPropertyFriendly, itemPropertyValue, itemPropertyDisplay in
          ItemInfo:ItemPropertyValuesForDisplay(item, BS_ITEM_INFO_DISPLAY_TYPE.TEXT)
        do
          -- Start this property with `<Friendly Property Name>: <Property Value>`.
          infoText = infoText
            .. string.format(L.Symbol_Colon, itemPropertyFriendly)
            .. " "
            .. itemPropertyDisplay
            .. BS_NEWLINE
          ...
          if type(ruleFunctionExamples) == "table" then
            if type(itemPropertyValue) == "table" then
              for _, val in ipairs(itemPropertyValue) do
                infoText = infoText
                  .. ItemInfo:BuildRuleFunctionExampleText(ruleFunctionExamples, val, itemPropertyFriendly)
              end
            else
              infoText = infoText
                .. ItemInfo:BuildRuleFunctionExampleText(ruleFunctionExamples, itemPropertyValue, itemPropertyFriendly)
            end
          else
