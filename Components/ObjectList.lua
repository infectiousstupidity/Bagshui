...
Bagshui:AddComponent(function()
  ...
  function ObjectList:New(instanceProperties)
    ...
    assert(
      type(instanceProperties.objectNamePlural) == "string",
      "instanceProperties.objectNamePlural must be a string"
    )
    ...
    assert(
      type(instanceProperties.defaults) == "table" or not instanceProperties.defaults,
      "instanceProperties.defaults must be a table"
    )
    ...
    local objectList = {
      ...
      objectChangeEvent = "BAGSHUI_"
        .. string.gsub(string.upper(instanceProperties.objectName), "%s", "_")
        .. "_UPDATE",
    ...
    objectList.idListSortFunctions = {
      name = function(idA, idB)
        return string.lower(objectList._idListSortFunctions_objectList[idA].name or "")
          < string.lower(objectList._idListSortFunctions_objectList[idB].name or "")
      end,

      builtin = function(idA, idB)
        return (
          (objectList._idListSortFunctions_objectList[idA].builtin and 1 or 0)
          .. string.lower(objectList._idListSortFunctions_objectList[idA].name or "")
        )
          < (
            (objectList._idListSortFunctions_objectList[idB].builtin and 1 or 0)
            .. string.lower(objectList._idListSortFunctions_objectList[idB].name or "")
          )
      end,
      ...
      inUse = function(idA, idB)
        return (
          (objectList._idListSortFunctions_objectList[idA].inUse and 1 or 0)
          .. string.lower(objectList._idListSortFunctions_objectList[idA].name or "")
        )
          < (
            (objectList._idListSortFunctions_objectList[idB].inUse and 1 or 0)
            .. string.lower(objectList._idListSortFunctions_objectList[idB].name or "")
          )
      end,
    }
    ...
    function objectList.idListSortWrapper(idA, idB)
      ...
      if not objectList._idListSortFunctions_objectList[idA] or not objectList._idListSortFunctions_objectList[idB] then
        return false
      end
  ...
  function ObjectList:Init(subclassWillSetInitializedProperty)
    ...
    if type(self.objectMigrationFunction) == "function" then
      self.objectMigrationFunction(self, (Bagshui.objectVersions[self.dataStorageKey] or 1))
    end
...
event == "BAGSHUI_SETTING_UPDATE"
            and (string.find(tostring(arg1), "^defaultProfile") or string.find(tostring(arg1), "^profile"))
          )
        )
  ...
  function ObjectList:LoadDefaults(objectList)
    ...
    if not self.disableObjectDeletion then
      for id, objectInfo in pairs(self.list) do
        if
          objectInfo.builtin
          and not foundDefaults[id]
          and not (type(self.disableObjectDeletionFunc) == "function" and self.disableObjectDeletionFunc(id))
        then
          self.list[id] = nil
        end
      end
  ...
  function ObjectList:Import(objectInfo, dependencyMap, force)
    ...
    for existingObjectId, existingObjectInfo in pairs(self.list) do
      if
        not existingObjectInfo.builtin and BsUtil.ObjectsEqual(objectInfo, existingObjectInfo, self.importIgnoreKeys)
      then
        Bagshui:Print(
          string.format(
            L.ObjectList_ImportReusingExisting,
            L[self.objectType],
            objectInfo.name,
            existingObjectInfo.name
          )
        )
  ...
  function ObjectList:UpdateSortedObjectLists()
    ...
    for id, object in pairs(self.list) do
      ...
      table.insert(self.filteredIdLists[object.builtin and "builtin" or "custom"], id)
    end
  ...
  function ObjectList:IsNameDuplicate(objectId, objectInfo, testCustomAgainstBuiltin)
    ...
    return (
      ...
      or (objectInfo.builtin and self.defaultObjectNames[objectInfo.name] ~= nil and self.defaultObjectNames[objectInfo.name] ~= objectId)
      or (
  ...
  function ObjectList:AddUse(objectMetadata, useType, left, right, position, leftPrefix)
    ...
    assert(
      type(leftPrefix) == "nil" or type(leftPrefix) == "string",
      "ObjectList:AddUse() [" .. self.objectType .. "] - leftPrefix must be a string or nil"
    )
    assert(
      type(objectMetadata[useType .. "Left"]) == "table",
      "ObjectList:AddUse() [" .. self.objectType .. "] - " .. useType .. " is not a valid uses useType"
    )
    ...
    elseif type(left) == "table" then
      for i = 1, table.getn(left) do
        self:AddUseHelper(
          ...
          useType,
          left[i],
          type(right) == "table" and right[i] or "",
          position,
          leftPrefix
        )
      end
  ...
  function ObjectList:InitUi(objectManagerClass, objectEditorClass, objectManagerParamOverrides)
      ...
      if not property or not objectListInstance.sortedIdListsForManager[property] then
        Bagshui:PrintError(
          self.objectType
            ...
            .. tostring(property)
            .. " not found! Using name sort instead (this shouldn't happen)."
        )
        property = "name"
      end
    ...
    local objectManagerParams = {
      ...
      managerListEntryOnEnter = function(this, modifierKeyRefresh)
        ...
        if _G.IsAltKeyDown() and table.getn(self.meta[this.bagshuiData.scrollableListEntry].usesOfProfilesLeft) > 0 then
        ...
        if type(tooltipStringsLeft) == "table" and table.getn(tooltipStringsLeft) > 0 then
          ...
          _G.GameTooltip:SetPoint("TOPRIGHT", this, "TOPLEFT")
          ...
          for i = 1, table.getn(tooltipStringsLeft) do
            thisLinePartCount = (string.len(tooltipStringsLeft[i]) > 0 and 1 or 0)
              + (string.len(tooltipStringsRight[i] or "") > 0 and 1 or 0)
            if thisLinePartCount ~= lastLinePartCount then
              _G.GameTooltip:AddDoubleLine(
                table.concat(listTooltipLinesLeft, BS_NEWLINE),
                (table.getn(listTooltipLinesRight) > 1 and table.concat(listTooltipLinesRight, BS_NEWLINE) or nil)
              )
              BsUtil.TableClear(listTooltipLinesLeft)
              BsUtil.TableClear(listTooltipLinesRight)
            end
          ...
          if
            _G.IsAltKeyDown()
            and table.getn(self.meta[this.bagshuiData.scrollableListEntry].usesLeft) > 0
          ...
          if altAction and altResult then
            _G.GameTooltip:AddLine(
              BS_NEWLINE .. GRAY_FONT_COLOR_CODE .. string.format(L.Symbol_Colon, altResult) .. " " .. altAction
            )
          end
          _G.GameTooltip:Show()
        end
