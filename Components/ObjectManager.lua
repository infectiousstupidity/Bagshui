...
Bagshui:AddComponent(function()
  ...
  function ObjectManager:New(props)
    ...
    assert(
      type(props.objectType) == "string",
      "Unable to instantiate ObjectManager: props.objectType is required and must be a string"
    )

    local name = props.objectType .. "Manager"

    assert(
      props.disableObjectCreation or type(props.objectTemplate) == "table",
      "Unable to instantiate " .. name .. " ObjectManager: props.objectTemplate is required and must be a table"
    )
    assert(
      props.disableObjectCreation or props.objectTemplate.name,
      "Unable to instantiate " .. name .. " ObjectManager: props.objectTemplate MUST have a Name property"
    )

    assert(
      (props.disableObjectCreation and props.disableObjectEditing) or type(props.editorFields) == "table",
      "Unable to instantiate " .. name .. " ObjectManager: props.editorFields is required and must be a table"
    )
    assert(
      (props.disableObjectCreation and props.disableObjectEditing) or type(props.editorFieldProperties) == "table",
      "Unable to instantiate " .. name .. " ObjectManager: props.editorFieldProperties is required and must be a table"
    )

    assert(
      props.listType == BS_UI_SCROLLABLE_LIST_TYPE.ITEM or props.managerColumns or props.managerDisplayProperty,
      "Unable to instantiate "
        .. name
        .. " ObjectManager: props.managerColumns or props.managerDisplayProperty is required"
    )
    ...
    local function managerColumnTextFunc(fieldName, objectId, objectInfo, preliminaryDisplayValue)
      ...
      if updatedDisplayValue == preliminaryDisplayValue then
        if fieldName == "builtin" then
          updatedDisplayValue = objectInfo[fieldName] ~= nil
              and (GRAY_FONT_COLOR_CODE .. "Bagshui" .. FONT_COLOR_CODE_CLOSE)
            or L.Custom
        end
        if fieldName == "inUse" then
          updatedDisplayValue = objectInfo[fieldName] and L.Yes
            or (GRAY_FONT_COLOR_CODE .. L.No .. FONT_COLOR_CODE_CLOSE)
        end
      end
  ...
  function ObjectManager:UpdateToolbarState() end
  ...
  function ObjectManager:NewObject(template, onFirstSave)
    if
      ...
      or (type(self.disableObjectCreationFunc) == "function" and self.disableObjectCreationFunc())
    then
      return
    end
  ...
  function ObjectManager:DeleteAfterConfirmation()
      ...
      _G.StaticPopupDialogs[dialogName] = {
        ...
        OnAccept = function(data)
          for objectId, _ in pairs(data.selectedObjects) do
            -- Final safety check.
            if
              not (type(self.disableObjectDeletionFunc) == "function" and self.disableObjectDeletionFunc(objectId))
            then
    ...
    _G.StaticPopupDialogs[dialogName].text = string.format(
      ...
      objectsToDelete,
      (
        self.deletePromptExtraInfo
          and (BS_NEWLINE .. GRAY_FONT_COLOR_CODE .. self.deletePromptExtraInfo .. FONT_COLOR_CODE_CLOSE)
        or ""
      )
    )
  ...
  function ObjectEditor:InitUi()
    ...
    for fieldIndex, fieldName in ipairs(self.editorFields) do
      ...
      if fieldProps and not fieldProps.hidden then
        assert(
          self.objectTemplate[fieldName],
          "objectTemplate property for " .. self.name .. " is missing " .. fieldName
        )
        ...
        elseif fieldType == "ScrollableList" or fieldType == "ItemList" then
          ...
          local scrollableListType = fieldProps.scrollableListType
          ...
          if fieldType == "ItemList" then
            table.insert(createScrollableListParams.buttons, {
              scrollableList_ButtonName = BS_UI_SCROLLABLE_LIST_BUTTON_NAME.SHARE,
              scrollableList_AutomaticAnchor = true,
              anchorPoint = "TOPLEFT",
              anchorToPoint = "BOTTOMLEFT",
          ...
          -- Create the widget.
          self.scrollFrames[fieldName], self.scrollChildren[fieldName], self.lists[fieldName] =
            ui:CreateScrollableList(createScrollableListParams)
          widget = self.scrollFrames[fieldName]
    ...
    if longestLabel < labelWidth then
      ...
      for _, labelWidgetPair in pairs(self.labelWidgetPairs) do
        ...
        ui:SetWidth(
          labelWidgetPair.bagshuiData.widget,
          labelWidgetPair.bagshuiData.initialWidgetWidth + labelDifference
        )
      end
  ...
  function ObjectEditor:PopulateField(fieldName, fallbackStorageTable)
    ...
    local readOnly = (
      self.originalObject.readOnly == true or self.manager.editorFieldProperties[fieldName].readOnly == true
    )

    -- Edit boxes and scrollable lists are different.
    if self.editBoxes[fieldName] then
      -- Use our `:SetText()` wrapper to handle read-only.
      self.ui:SetEditBoxText(self.editBoxes[fieldName], value or "", readOnly)
    elseif self.lists[fieldName] then
      ...
      for _, button in pairs(self.lists[fieldName].bagshuiData.buttons) do
        button.bagshuiData.scrollableList_Disable = button.bagshuiData.scrollableList_DisableIfReadOnly and readOnly
          or false
      end
  ...
  function ObjectEditor:IsObjectDirty(original, updated)
    ...
    if
      ...
      or (type(original) == "nil" and type(updated) == "table" and BsUtil.TrueTableSize(updated) == 0)
    then
    ...
    if
      ...
      or (type(original) == "nil" and type(updated) == "string" and string.len(updated) == 0)
    then
  ...
  function ObjectEditor:SetTitle()
    ...
    if string.len(objectName) > 0 then
      title = string.format(L.Symbol_Colon, title)
        .. " "
