...
self._inventoryClassInstance
          ...
          and self["profile" .. BS_PROFILE_TYPE[arg2.profileScope]]
            == arg4["profile" .. BS_PROFILE_TYPE[arg2.profileScope]]
        )
      )
    then
    ...
    if settingInfo.scope == BS_SETTING_SCOPE.INVENTORY then
      ...
      if self._inventoryClassInstance then
        ...
        if settingInfo.profileScope then
          ...
          settingTable = self._inventoryClassSettingsSource
  ...
  function Settings:Get(settingName, includeVersion, noCache, noDefault)
    ...
    if includeVersion then
      return value,
        ((settingTable and settingTable[SETTING_VERSION_KEY] and settingTable[SETTING_VERSION_KEY][settingName]) or 0)
    end
  ...
  function Settings:Set(settingName, value, skipValidation, force)
    ...
    if self._initialized then
      Bagshui:RaiseEvent("BAGSHUI_SETTING_UPDATE", nil, settingName, settingInfo, finalValue, self)
    end
  end
...
settingInfo.scope ~= BS_SETTING_SCOPE.INVENTORY
          or (self._inventoryClassInstance ~= nil and self:GetTableForSetting(settingInfo))
        )
...
(
            ...
            if currentValue ~= newValue then
              ...
              if not silent and currentValue ~= nil then
                qualifiedSettingName = tostring(
                  (settingInfo.scope ~= BS_SETTING_SCOPE.INVENTORY) and settingInfo.scope or self._inventoryType
                ) .. "." .. settingName
  ...
  function Settings:ValidateColorRgba(value, settingInfo)
    local validatedValue = value
    if
      type(validatedValue) ~= "table"
      or (type(validatedValue) == "table" and table.getn(validatedValue) < table.getn(settingInfo.defaultValue))
    then
