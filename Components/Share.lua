...
Bagshui:AddComponent(function()
  ...
  function Share:AddTrackedClass(objectList)
    assert(
      type(objectList) == "table" and objectList._super == Bagshui.prototypes.ObjectList,
      "Share: the given objectList does not appear to be a Bagshui ObjectList instance."
    )
  ...
  function Share:GetObjectInfo(objectTypeAndId)
    ...
    -- like refactoring.
    return self.objectLists[objectInfo[1]].list[objectInfo[2]], objectInfo[2], self.objectLists[objectInfo[1]]
  end
  ...
  function Share:Export(selectedEntries)
    ...
    if not ui.dialogProperties[dialogName] then
      ui:AddMultilineDialog(dialogName, {
        ...
        dialogCustomizationFunc = function(dialog)
          ...
          dialog.uiFrame.bagshuiData.encodeExplanation =
            dialog.uiFrame:CreateFontString(nil, nil, "GameFontNormalSmall")
          ...
          dialog.uiFrame.bagshuiData.encodeExplanation:SetWidth(
          ...
          dialog.uiFrame.bagshuiData.encodeCheckbox:SetPoint(
            ...
            dialog.uiFrame.bagshuiData.encodeExplanation,
            "TOPLEFT",
            -dialog.uiFrame.bagshuiData.encodeCheckbox:GetWidth(),
            -2
          )

          -- Re-anchor bottom of ScrollFrame to encode checkbox.
          ui:SetPoint(
            dialog.uiFrame.bagshuiData.scrollFrame,
            "BOTTOM",
            dialog.uiFrame.bagshuiData.encodeCheckbox,
            "TOP",
            0,
            4
          )

          dialog.dialogProperties.height = dialog.dialogProperties.height
            + dialog.uiFrame.bagshuiData.encodeExplanation:GetHeight()
            + dialog.uiFrame.bagshuiData.encodeCheckbox:GetHeight()
        end,
      })
    end
    ...
    local dialog = ui:ShowDialog(dialogName, nil, nil, nil, nil, self.objectManager.uiFrame)
  ...
  function Share:AddToExport(objectList, objectId)
    assert(
      type(objectList) == "table" and objectList.Export,
      "Share:AddToExport() -- objectList does not appear to be a Bagshui ObjectList instance."
    )
  ...
  function Share:OpenForExport(sourceObjectManager, objectIds)
    ...
    if
      sourceObjectManager
      and sourceObjectManager.objectType
      and objectIds
      and self.objectManager
      and self.objectManager.ui
      and self.objectManager.ui.listFrame
    then
