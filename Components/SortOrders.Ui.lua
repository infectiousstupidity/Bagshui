  ...
  function SortOrders:InitUi()
          ...
          fields = {
            ...
            entryFrameCreationFunc = function(entryFrameName, entryFrame, ui)
              ...
              local directionButton = ui:CreateIconButton({
                ...
                onClick = function()
                  entryFrame.bagshuiData.scrollableListEntry.direction = string.lower(
                    entryFrame.bagshuiData.scrollableListEntry.direction or ""
                  ) == "desc" and "asc" or "desc"
              ...
              local reverseButton = ui:CreateIconButton({
                ...
                onClick = function()
                  entryFrame.bagshuiData.scrollableListEntry.reverseWords = not entryFrame.bagshuiData.scrollableListEntry.reverseWords
                      and true
                    or nil
                  ui:PopulateScrollableList(entryFrame.bagshuiData.listFrame, nil, true, true)
                end,
    ...
    function sortingEditor:GetSortFieldInfo(entry)
      ...
      if not sortFieldInfo then
        Bagshui:PrintError(
          "Sort field `"
            .. BsSortOrders:GetFieldIdentifier(entry.field, entry.lookup, " ")
            .. "` not found while updating Sort Order editor UI. This shouldn't happen!"
        )
        return entry
      end
