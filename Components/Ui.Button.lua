...
Bagshui:AddComponent(function()
  ...
  function Ui:CreateButton(name, parent, buttonText, onClick)
    ...
    local element = _G.CreateFrame("Button", elementName, parent, "UIPanelButtonTemplate")
  ...
  function Ui:CreateCheckbox(name, parent, template, buttonText, onClick, width, height)
    ...
    local element = _G.CreateFrame("CheckButton", elementName, parent, template)
  ...
  function Ui:CreateDropDownMenuButton(name, parent, width, menusInstance, menuType, openFuncArg2)
    ...
    local dropDown = _G.CreateFrame("Frame", dropDownName, parent, "UIDropDownMenuTemplate")
  ...
  function Ui:CreateIconButton(buttonOpts)
    ...
    self:SetIconButtonTexture(
      button,
      (
        (string.find(buttonOpts.texture, "\\")) and buttonOpts.texture
  ...
  function Ui:SetIconButtonColors(button, vertexColor, hoverVertexColor)
    assert(
      type(button) == "table" and button.GetNormalTexture,
      "Ui:SetIconButtonColor() - button does not appear to be a WoW UI button"
    )
    ...
    button
      :GetNormalTexture()
      :SetVertexColor(r, g, b, (      -- NormalTexture should be faded while the cooldown is active since
      -- the cooldown takes on its opacity.
(button.bagshuiData and button.bagshuiData.isOnCooldown) and 0.4 or vertexColor[4] or 1))
  ...
  function Ui:UpdateIconButtonCooldown(button)
    ...
    if
      ...
      and not (button:IsEnabled() or button.bagshuiData.isOnCooldown)
    then
      button.bagshuiData.cooldownTexture:Hide()
      return
    end
    ...
    button.bagshuiData.cooldownRemaining = button.bagshuiData.cooldownDuration
      - (_G.GetTime() - button.bagshuiData.cooldownStart)
    ...
    if button.bagshuiData.cooldownRemaining <= 0 then
      ...
      button.bagshuiData.cooldownPercentRemaining = button.bagshuiData.cooldownRemaining
        / button.bagshuiData.cooldownDuration
      button.bagshuiData.cooldownTexture:SetTexCoord(0, 1, button.bagshuiData.cooldownPercentRemaining, 1)
      button.bagshuiData.cooldownTexture:SetPoint(
        "TOP",
        button,
        "BOTTOM",
        0,
        (button:GetHeight() * (1 - button.bagshuiData.cooldownPercentRemaining))
      )
  ...
  function Ui:ShineIconButton(button)
    ...
    button.bagshuiData.shineFrame.bagshuiData.shine:SetHeight(
      button.bagshuiData.shineFrame:GetHeight() * ICON_BUTTON_SHINE_SCALE
    )
    button.bagshuiData.shineFrame.bagshuiData.shine:SetWidth(
      button.bagshuiData.shineFrame:GetWidth() * ICON_BUTTON_SHINE_SCALE
    )
...
bagshuiInfo.noTooltipTextDelay
          ...
          or (bagshuiInfo.tooltipTextDelay and secondPass)
        )
      then
