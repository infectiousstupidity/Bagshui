...
Bagshui:LoadComponent(function()
  ...
  local function ItemButton_OnEnter(targetItemButton, fromEntryFrame, refreshTooltipOnly)
    ...
    if type(itemString) == "string" and string.find(itemString, "^item:") then
      ...
      _G.GameTooltip.bagshuiData.showInfoTooltipWithoutAlt = this.bagshuiData
        and this.bagshuiData.showBagshuiInfoWithoutAlt

      -- There's a custom tooltip position.
      if
        not this.bagshuiData.tooltipAnchorDefault
        and (this.bagshuiData.tooltipAnchor == "ANCHOR_NONE" or this.bagshuiData.tooltipAnchor == "ANCHOR_PRESERVE")
  ...
  function ItemButton_OnEnter_FixTooltipPosition(button, xAnchor)
    if (BsInfoTooltip:GetBottom() or 0) <= (_G.UIParent:GetBottom() or 0) then
      ...
      _G.GameTooltip:ClearAllPoints()
      _G.GameTooltip:SetPoint("BOTTOM" .. xAnchor, BsInfoTooltip, "TOP" .. xAnchor, 0, -BsSkin.infoTooltipYOffset)
    end
  end
  ...
  function Ui:SkinItemButton(button, buttonType)
    ...
    if BsSkin.itemSlotIconTexCoord then
      buttonComponents.iconTexture:SetTexCoord(
        BsSkin.itemSlotIconTexCoord[1],
        BsSkin.itemSlotIconTexCoord[2],
        BsSkin.itemSlotIconTexCoord[3],
        BsSkin.itemSlotIconTexCoord[4]
      )
    end
    ...
    buttonComponents.count:SetPoint(
      "BOTTOMRIGHT",
      button,
      -BsSkin.itemSlotDecorationAnchor,
      BsSkin.itemSlotDecorationAnchor
    )
    buttonComponents.stock:SetPoint(
      "TOPLEFT",
      button,
      BsSkin.itemSlotDecorationAnchor,
      -BsSkin.itemSlotDecorationAnchor
    ...
    buttonComponents.border:SetPoint(
      "TOPLEFT",
      button,
      "TOPLEFT",
      -BsSkin.itemSlotBorderAnchor,
      BsSkin.itemSlotBorderAnchor
    )
    buttonComponents.border:SetPoint(
      "BOTTOMRIGHT",
    ...
    buttonInfo.borderBackdropTable = {
      ...
      insets = {
        left = BsSkin.itemSlotInset,
        right = BsSkin.itemSlotInset,
        top = BsSkin.itemSlotInset,
        bottom = BsSkin.itemSlotInset,
      },
    }
    ...
    if BsSkin.itemSlotHighlightAnchor then
      ...
      buttonComponents.highlightTexture:SetPoint(
        "TOPLEFT",
        -BsSkin.itemSlotHighlightAnchor,
        BsSkin.itemSlotHighlightAnchor
      )
      buttonComponents.highlightTexture:SetPoint(
        "BOTTOMRIGHT",
    ...
    buttonComponents.innerGlow:SetPoint(
      "TOPLEFT",
      button,
      "TOPLEFT",
      -BsSkin.itemSlotInnerGlowAnchor,
      BsSkin.itemSlotInnerGlowAnchor
    )
    buttonComponents.innerGlow:SetPoint(
      "BOTTOMRIGHT",
    ...
    if buttonType == BS_UI_ITEM_BUTTON_TYPE.ITEM then
      ...
      buttonComponents.qualityBadge =
        self:CreateShadowedTexture(button, "Interface\\TargetingFrame\\UI-RaidTargetingIcons")
      ...
      buttonComponents.qualityBadge:SetPoint(
        "BOTTOMLEFT",
        button,
        "BOTTOMLEFT",
        BsSkin.itemSlotDecorationAnchor,
      ...
      buttonComponents.topLeftBadge:SetPoint(
        "TOPLEFT",
        button,
        "TOPLEFT",
        BsSkin.itemSlotDecorationAnchor,
      ...
      buttonComponents.stockBadge:SetPoint(
        "TOPRIGHT",
        button,
        "TOPRIGHT",
        -BsSkin.itemSlotDecorationAnchor,
  ...
  function Ui:AssignItemToItemButton(button, item, groupId)
    ...
    if settings.itemUsableColors or settings.itemUsableBadges or settings.colorblindMode then
      ...
      if buttonInfo.topLeftBadgeAlpha == 0 then
        ...
        if (settings.itemUsableBadges or settings.colorblindMode) and usableTextureKey then
          self:SetShadowedTexture(buttonComponents.topLeftBadge, BS_INVENTORY_USABLE_BADGE[usableTextureKey].texture)
          self:SetShadowedTextureTexCoord(
            buttonComponents.topLeftBadge,
    ...
    if item.emptySlot == 1 then
      ...
      if inventory and inventory.settings and not inventory.settings.emptySlotBackgroundImage then
...
(
    ...
    if item and (item.emptySlot or 0) == 1 then
      ...
      -- Filled slot.
      buttonComponents.normalTexture:SetVertexColor(1, 1, 1, opacityOverride or 1)
    end

    -- Background opacity for search miss (only really matters for empty slots).
    buttonComponents.background:SetAlpha(
      (not containerHighlight) and opacityOverride or (buttonInfo.backgroundTextureAlpha or 1)
    )
  end
  ...
  function Ui:UpdateItemButtonStockState(button)
    ...
    if button.bagshuiData.item and inventory and not inventory.editMode then
      ...
      then
        ...
        if
          (inventory.settings.itemStockBadges or inventory.highlightChanges)
          -- Don't show stock badge if it's disabled for this group.
          and (not (inventory.groups and inventory.groups[groupId]) or inventory.groups[groupId].hideStockBadge ~= true)
        then
          ...
          if
            timeElapsed > (inventory.settings.itemStockChangeExpiration - inventory.settings.itemStockBadgeFadeDuration)
          then
