...
Bagshui:AddComponent(function()
  ...
  function Ui:CreateWindowFrame(name, parent, height, width, title, noCloseOnEscape)
    ...
    local frame = _G.CreateFrame("Frame", self:CreateElementName(name), frameParent)
    frame.bagshuiData = {
    ...
    if title then
      frame.bagshuiData.header, frame.bagshuiData.title, frame.bagshuiData.closeButton =
        self:CreateTitleBar(frame, title)
    end
  ...
  function Ui:CreateScrollableEditBoxWindow(
    name,
    parent,
    height,
    width,
  ...
  function Ui:CreateTitleBar(windowFrame, windowTitleText, closeFunction)
    local header = self:CreateHeaderFooter("Header", windowFrame, "TOP", BsSkin.windowTitleBarHeight)

    local title = self:CreateWindowTitle("Title", header, windowTitleText, "LEFT", 0, 0)

    local closeButton = self:CreateCloseButton("CloseButton", header, "RIGHT", 0, 0, closeFunction)
  ...
  function Ui:CreateHeaderFooter(name, windowFrame, anchorTopBottom, height, xOffset, yOffset)
    ...
    local headerFooter = _G.CreateFrame("Frame", self:CreateElementName(name), windowFrame)
    headerFooter:SetHeight(height or BsSkin.windowHeaderFooterHeight)

    -- Set position.
    headerFooter:SetPoint(anchorTopBottom .. "LEFT", windowFrame, xOffset, yOffset)
    headerFooter:SetPoint(anchorTopBottom .. "RIGHT", windowFrame, -xOffset, yOffset)
  ...
  function Ui:CreateCloseButton(name, windowFrame, anchor, xOffset, yOffset, clickFunction)
    local closeButton = _G.CreateFrame("Button", self:CreateElementName(name), windowFrame)
    ...
    closeButton:SetHitRectInsets(
      BsSkin.closeButtonHitRectInsets,
      BsSkin.closeButtonHitRectInsets,
      BsSkin.closeButtonHitRectInsets,
      BsSkin.closeButtonHitRectInsets
    )
      ...
      if type(clickFunction) ~= "function" then
        ...
        if parentWindow then
          ...
          Bagshui:PrintWarning(
            "UI WARNING: No parent frame found for close button '"
              .. tostring(name)
              .. "' (child of frame "
              .. tostring(windowFrame:GetName())
              .. ") -- frame may not be closable!"
          )
        end
      end
  ...
  function Ui:SetWindowShade(window, shadeOn, text)
    ...
    if shadeOn then
      if not window.bagshuiData.shadeFrame then
        ...
        local inset = (type(BsSkin.frameDefaultBorderStyle) == "table" and BsSkin.frameDefaultBorderStyle.insets)
