...
Bagshui:LoadComponent(function()
  ...
  function Util.Split(str, delimiter, delimiterIsPattern, includeEmpty)
    assert(
      type(str) == "string",
      "Util.Split() - str must be a string (was " .. type(str) .. ": " .. tostring(str) .. ")"
    )
  ...
  function Util.MatchLocalizedOrNon(str, strToMatch)
    ...
    return string.lower(Util.Trim(str)) == string.lower(strToMatch)
  ...
  function Util.Round(num, decimals)
    ...
    return tonumber(string.format("%." .. (decimals or 2) .. "f", num))
  end
  ...
  function Util.TableCopy(source, dest, forceKeyValueCopy, template, tableWildcard, parentHadTableWildcard)
    ...
    assert(
      dest == nil or type(dest) == "table",
      "TableCopy:() type(dest) = " .. type(dest) .. " instead of table or nil"
    )
    ...
    if table.getn(source) > 0 and not forceKeyValueCopy then
      ...
      if template and not parentHadTableWildcard then
        ...
        for key, templateVal in pairs(template) do
          ...
          elseif
            type(templateVal) == "table"
            and table.getn(templateVal) == 1
            and source[key]
            and templateVal ~= tableWildcard
          then
  ...
  function Util.TableInsertNonNil(tbl, val, emptyStringAsNil)
    if type(tbl) ~= "table" or val == nil or (emptyStringAsNil and type(val) == "string" and string.len(val) == 0) then
      return false
    end
  ...
  function Util.FlipAnchorPointComponent(point, componentNum)
    ...
    local searchAnchor = ((componentNum == 1 or not componentNum) and "^" or "")
      .. "%s"
      .. (componentNum == 2 and "$" or "")
  ...
  function Util.GetAnchorLeftRight(point)
    return (
      (point ~= "BOTTOM" and point ~= "TOP") and (point and (string.find(point, "RIGHT$") and "RIGHT" or "LEFT"))
      or ""
    )
  end
  ...
  function Util.GetFrameOffscreenAmount(frame, xy)
    assert(
      frame and frame.GetScale,
      "Util.GetFrameOffscreenAmount(): frame does not appear to be a valid WoW UI frame."
    )
    xy = xy or "x"
  ...
  function Util.ObjectsEqual(obj1, obj2, ignoreKeys, keyPath, maxDepth)
    ...
    for key2, _ in pairs(obj2) do
      if not (ignoreKeys and ignoreKeys[keyPrefix .. key2]) and obj1[key2] == nil then
        return false
      end
    end
