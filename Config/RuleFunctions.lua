...
Bagshui:AddComponent(function()
  ...
  Bagshui.config.RuleFunctions = {
    ...
    -- Item can be equipped at the specified location.
    {
      ...
      ruleFunction = function(rules, ruleArguments)
        if table.getn(ruleArguments) == 0 then
          ...
          return rules:TestItemAttribute("equipLocation", ruleArguments)
      ...
      ruleFunction = function(rules, ruleArguments)
        ...
        for _, categoryId in ipairs(ruleArguments) do
          ...
          if BsUtil.TableContainsValue(rules._matchCategory_callStack, categoryId) then
            ...
            rules[errorProperty] = BsUtil.Trim(L.Error_Rule_MatchCategory_Loop)
              .. " "
    ...
    -- Item is in the specified inventory location.
    {
      ...
      ruleFunction = function(rules, ruleArguments)
        ...
        for i, arg in ipairs(ruleArguments) do
          ...
          elseif
            arg == string.lower(L[BS_INVENTORY_TYPE.KEYRING])
            or arg == "keyring"
            or arg == "keychain"
            or arg == "keys"
          then
        ...
        for outfitType, outfits in pairs(outfitterOutfits) do
          for outfitId, outfit in pairs(outfits) do
            ...
            for slot, item in pairs(outfit.Items) do
              if
                item.Code == tonumber(code) and item.SubCode == tonumber(subCode)
    ...
    -- Item is of the specified quality.
    {
      ...
      ruleFunction = function(rules, ruleArguments)
        return rules:TestItemAttribute("quality", ruleArguments, "string,number")
    ...
    -- Does the tooltip contain "Class: <Class Name>"?
    {
      ...
      ruleFunction = function(rules, ruleArguments)
        ...
        for _, arg in ipairs(ruleArguments) do
          ...
          rules._requiresClass_Tooltip[1] =
            string.format(rules._requiresClass_BasePattern, ".*%f[%a]" .. arg .. "%f[%A].*")
          rules._requiresClass_Tooltip[2] = string.format(
            rules._requiresClass_BasePattern,
            ".*%f[%a]"
              .. (BsGameInfo.lowercaseToNormalCaseLocalizedCharacterClasses[string.lower(arg)] or arg)
              .. "%f[%A].*"
          )
